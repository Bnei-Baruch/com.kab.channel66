...
public class HLSEvents {
	public class Url
	{
		...
		public Url(String val, String quality,String quality_name,String type)
		{
			url_quality = quality;
			url_quality_name = quality_name;
			url_value = val;
			type_val = type;
	...
	private static final String URL_QUALITY_NAME = "url_quality_name";
			...
			for (int i=0;i<mLocale.length();i++)
			{
				...
				for(int j=0;j<pages.length();j++)
				{
					...
					for(int k=0;k<urls.length();k++)
					{
						...
						try {
							quality_name = new String(url.getString(URL_QUALITY_NAME).getBytes("ISO-8859-1"), "UTF-8");
						} catch (UnsupportedEncodingException e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
						}
						type = url.getString(URL_TYPE);
						page_c.urls.urlslist.add(new Url(val,quality,quality_name,type));
					}
